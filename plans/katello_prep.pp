  ...
  if !$major_packages_check.ok or $force {
    ...
    if $katello_role {
      ...
      if $db_manage {
        ...
        $postgresql_rpm_version = $rpm_version_query.value['stdout'] ? {
          /[0-9]/ => Float($rpm_version_query.value['stdout']),
          default => Float('0.0')
        }
      ...
      if ($postgresql_install or ($db_manage and $force)) {
        ...
        $installed_postgresql_version = $new_rpm_version_query.value['stdout'] ? {
          /[0-9]/ => Float($new_rpm_version_query.value['stdout']),
          default => Float('0.0')
        }
      ...
      run_task('boltello::remove_packages', 
        $nodes, 
        packages => 'postgresql postgresql-server mongodb',
      )
